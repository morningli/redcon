...
func AppendArray(b *Buffer, n int) {
...
func AppendError(b *Buffer, s string) {
	_, _ = b.Write([]byte{'-'})
	_, _ = b.Write([]byte(stripNewlines(s)))
	_, _ = b.Write([]byte{'\r', '\n'})
}
...
func AppendAny(b *Buffer, v interface{}) {
	switch v := v.(type) {
	case SimpleString:
